...
auto InRange = [](int b, int e) {
    return [=](int x) -> bool { return x <= e && x >= b; };
};

auto OR = [](std::function<bool(int)> f, std::function<bool(int)> g) {
    return [=](int x) -> bool { return f(x) || g(x); };
};

auto IsDigit = InRange('0', '9');
auto IsLower = InRange('a', 'z');
auto IsUpper = InRange('A', 'Z');
auto IsAlpha = OR(IsLower, IsUpper);
...
shared_ptr<Expr> IdHandler::IdFactory::CreateExpr(
    string token, vector<shared_ptr<Expr>> arguments) {
    shared_ptr<IdExpr> p = make_shared<IdExpr>();
    p->name = token;
    return p;
}

shared_ptr<Expr> NumberHandler::NumberFactory::CreateExpr(
    string token, vector<shared_ptr<Expr>> arguments) {
...
shared_ptr<Expr> BinaryHandler::BinaryFactory::CreateExpr(
    string token, vector<shared_ptr<Expr>> arguments) {
    shared_ptr<BinaryExpr> p = make_shared<BinaryExpr>();
    switch (token[0]) {
        case '+':
            p->op = BinaryOperator::Plus;
            break;
        case '-':
            p->op = BinaryOperator::Minus;
            break;
        case '*':
            p->op = BinaryOperator::Multiply;
            break;
        case '/':
            p->op = BinaryOperator::Divide;
            break;
    }
    p->first = arguments[0];
...
shared_ptr<Expr> InvokeHandler::InvokeFactory::CreateExpr(
    string token, vector<shared_ptr<Expr>> arguments) {
